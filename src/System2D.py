...
class Cell(object):
    ...
    def set_face_vectors(self, nface, facelist):   #, n, FaceCellMap):
        for i in range(self.N):
            #thing = self.nodes[i] + self.nodes[(i+1)%self.N]
            #thing = 0.5 * (self.nodes[i] + self.nodes[(i+1)%self.N] )
            #print(thing)
...
class Grid(object):
    ...
    def write_grid_to_vtk(self,filename_vtk='grid.vtk'):
        '''
        #*******************************************************************************
        ...
        for k, vtx in enumerate(self.nodes):
        ...
        if ntria>0:
            ...
            for i in range(ntria):
                lines.append('3 '+ 
                             str(self.cells[i].nodes[0].nid) +' ' 
                             +str(self.cells[i].nodes[1].nid) +' '  
                             +str(self.cells[i].nodes[2].nid)
                             +'\n'
                             )
